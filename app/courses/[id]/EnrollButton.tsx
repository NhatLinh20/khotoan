...
export default function EnrollButton({ courseId, isEnrolled, isLoggedIn, price }: EnrollButtonProps) {
 ...
 if (enrolled) {
 return (
 <div className="w-full flex items-center justify-center gap-3 bg-emerald-50 border-2 border-emerald-500 text-emerald-700 py-2.5 rounded-md font-display font-bold text-lg">
 <CheckCircle size={22} />
 Đã đăng ký học
 </div>
 )
 ...
 disabled={isPending}
 className="w-full flex items-center justify-center gap-3 bg-primary hover:bg-primary-dark disabled:opacity-60 text-surface py-2.5 rounded-md font-display font-bold text-lg transition-all hover:scale-[1.02] active:scale-95 shadow-xl shadow-primary/30"
 >
 {isPending ? (
 ...
 {!isLoggedIn && (
 <p className="text-center text-[11px] text-secondary/80 font-medium">
