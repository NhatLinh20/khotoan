...
const selectCls =
 'w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-md text-sm text-surface appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all'
const labelCls = 'text-[10px] font-display font-bold text-secondary/80 uppercase tracking-widest mb-1.5 flex items-center gap-1.5'
...
export default function CourseContentAccordion({
 ...
 <span className="w-5 h-5 rounded-full bg-primary/20 text-primary flex items-center justify-center text-[9px] font-display font-bold shrink-0">①</span>
 Chương
 </p>
 ...
 <div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-secondary/80">
 ...
 className="mt-2 w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg bg-primary text-surface text-[11px] font-bold hover:bg-primary/90 transition-colors"
 >
 Học bài này <ChevronRight size={12} />
 </button>
 )}
 {!isEnrolled && (
 <p className="mt-2 text-[10px] text-secondary font-medium">
 ...
 : 'hover:bg-slate-800/50 text-secondary/80 hover:text-gray-200'
 }`}
 onClick={() => {
 ...
 >
 <span className="text-[10px] font-display font-bold w-5 shrink-0">
 ...
 <span className="text-[9px] font-display font-bold">{parsePdfUrls(lesson.pdf_url).length}</span>
 )}
 </span>
 )}
 </div>
