...
export default function ExamBankBuilder({ initialExamId, initialData }: Props) {
 ...
 className="inline-flex items-center gap-2 px-4 py-2 rounded-md border border-secondary/20 text-sm font-bold text-secondary hover:bg-neutral :bg-slate-800 transition-all disabled:opacity-50">
 {isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}
 Lưu nháp
 </button>
 <button onClick={() => handleSave(true)} disabled={isPending}
 className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-primary text-surface text-sm font-bold hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 disabled:opacity-50">
 ...
 <h2 className="text-sm font-display font-bold text-primary">Thông tin đề thi</h2>
 ...
 {/* Scoring Table */}
 <div className="bg-surface rounded-md border border-secondary/20 p-5 space-y-4">
 <h2 className="text-sm font-display font-bold text-primary">Cấu trúc điểm</h2>
 <div className="overflow-x-auto">
 <table className="w-full text-sm">
 <thead>
 <tr className="border-b border-secondary/20">
 <th className="py-2 text-left font-bold text-secondary text-xs">Loại câu</th>
 <th className="py-2 text-center font-bold text-secondary text-xs">Số câu</th>
 <th className="py-2 text-right font-bold text-secondary text-xs w-28">Tổng điểm</th>
 </tr>
 </thead>
 ...
 <td className="py-2">
 <input type="number" step="any" min={0} value={ptsMC} onChange={e => setPtsMC(Number(e.target.value) || 0)} className="w-full text-right bg-neutral border border-secondary/20 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary/20" />
 </td>
 </tr>
 <tr>
 ...
 <td className="py-2">
 <input type="number" disabled value={countTF * 1.0} className="w-full text-right bg-neutral text-secondary/80 border border-secondary/20 rounded-lg px-2 py-1 cursor-not-allowed" />
 </td>
 </tr>
 <tr>
 ...
 <td className="py-2">
 <input type="number" step="any" min={0} value={ptsShort} onChange={e => setPtsShort(Number(e.target.value) || 0)} className="w-full text-right bg-neutral border border-secondary/20 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary/20" />
 </td>
 </tr>
 <tr>
 ...
 <td className="py-2">
 <input type="number" step="any" min={0} value={ptsEssay} onChange={e => setPtsEssay(Number(e.target.value) || 0)} className="w-full text-right bg-neutral border border-secondary/20 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary/20" />
 </td>
 </tr>
 </tbody>
 <tfoot className="border-t-2 border-secondary/20">
 <tr>
 <td className="py-2 font-display font-bold">Tổng cộng</td>
 <td className="py-2 text-center font-display font-bold">{examQuestions.length}</td>
 <td className="py-2 text-right font-display font-bold text-primary">{totalScore} đ</td>
 </tr>
 </tfoot>
 </table>
 </div>
 ...
 <h2 className="text-sm font-display font-bold text-primary">Câu hỏi trong đề</h2>
 <div className="flex items-center gap-2">
 <span className="text-xs font-bold text-secondary bg-neutral px-2 py-0.5 rounded-lg">{examQuestions.length} câu</span>
 <span className="text-xs font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-lg">{totalScore} đ</span>
 </div>
 </div>
 {examQuestions.length === 0 ? (
 <div className="py-10 text-center text-secondary/80 text-sm">
 ...
 <div key={q._uid} className="flex items-center gap-2 px-3 py-2.5 hover:bg-neutral/50 :bg-slate-800/30 group">
 <span className="text-[10px] font-display font-bold text-secondary/50 w-5 text-center shrink-0">{i + 1}</span>
 <div className="flex-1 min-w-0">
 <p className="text-[11px] font-display font-bold text-secondary font-mono">{q.question_code}</p>
 <p className="text-[11px] text-secondary line-clamp-1 mt-0.5">{q.content.slice(0, 60)}…</p>
 </div>
 <span className={`text-[9px] font-display font-bold px-1.5 py-0.5 rounded shrink-0 ${TYPE_COLORS[q.type]}`}>{q.type.toUpperCase()}</span>
 <span className="text-[10px] font-bold text-secondary w-10 text-right shrink-0">{Number(getQuestionScore(q.type).toFixed(2))}đ</span>
 <button onClick={() => handleRemove(q._uid)} className="p-1 rounded-lg text-red-400 hover:bg-red-50 :bg-red-900/20 opacity-0 group-hover:opacity-100 transition-all shrink-0">
 <X size={12} />
 </button>
 </div>
 ))}
 ...
 {/* RIGHT: Question bank */}
 <div className="lg:col-span-8 bg-surface rounded-md border border-secondary/20 overflow-hidden">
 <div className="px-4 py-3 border-b border-secondary/20">
 <h2 className="text-sm font-display font-bold text-primary mb-3">Tìm câu hỏi từ ngân hàng</h2>
 <div className="flex flex-col gap-2">
 {/* Search */}
 <div className="relative">
 <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-secondary/80" />
 <input
 value={codeSearch} onChange={e => setCodeSearch(e.target.value.toUpperCase())}
 placeholder="Tìm theo mã ID câu hỏi..."
 className="w-full pl-9 pr-4 py-2 bg-neutral border border-secondary/20 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 font-mono"
 />
 </div>
 {/* Filters */}
 ...
 <div className="py-10 text-center text-secondary/80 text-sm">Không tìm thấy câu hỏi nào</div>
 ) : (
 <table className="w-full text-sm">
 <thead>
 <tr className="bg-neutral/50 border-b border-secondary/20">
 <th className="px-3 py-2 text-left text-[10px] font-display font-bold text-secondary/80 uppercase w-28">Mã ID</th>
 <th className="px-3 py-2 text-left text-[10px] font-display font-bold text-secondary/80 uppercase">Nội dung</th>
 <th className="px-3 py-2 text-center text-[10px] font-display font-bold text-secondary/80 uppercase w-20">Loại</th>
 <th className="px-3 py-2 text-center text-[10px] font-display font-bold text-secondary/80 uppercase w-20">Mức độ</th>
 ...
 return (
 <tr key={q.id} className={`hover:bg-neutral/50 :bg-slate-800/20 transition-colors ${already ? 'opacity-40' : ''}`}>
 <td className="px-3 py-2">
 <span className="font-mono text-[11px] font-display font-bold text-secondary">{q.question_code}</span>
 </td>
 <td className="px-3 py-2 max-w-xs">
 <p className="text-xs text-gray-700 line-clamp-2 leading-snug">{q.content}</p>
 </td>
 <td className="px-3 py-2 text-center">
 <span className={`text-[9px] font-display font-bold px-1.5 py-0.5 rounded ${TYPE_COLORS[q.type]}`}>{q.type.toUpperCase()}</span>
 </td>
 <td className="px-3 py-2 text-center">
 <span className={`text-[9px] font-display font-bold px-1.5 py-0.5 rounded ${DIFF_COLORS[q.difficulty]}`}>{q.difficulty}</span>
 </td>
 <td className="px-3 py-2 text-center">
 <button
 onClick={() => handleAdd(q)}
